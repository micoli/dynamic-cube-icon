...
export class CubeIcon {
	...
	constructor(element:any,opts:any){
		...
		opts.textSize = opts.textSize || 14;
		opts.textColor = opts.textColor || '#F7F7F7';
		opts.rotationStep = opts.rotationStep || 3
		...
		mouseChange(this.element,function(b,x,y){
			self.angle += ( (self.lastMouse[0]>x?-1:+1)*opts.rotationStep ) % 360;
			self.lastMouse = [x,y];
		...
		self.element.innerHTML = this.cssTpl( tplParameters ) +this.addCssTpl( tplParameters ) + this.htmlTpl( tplParameters );
	...
	public show(){
		(<HTMLDivElement>document.querySelector('#'+this.uuid+' .cube-wrap')).style.display='block';
	}

	public hide(){
		(<HTMLDivElement>document.querySelector('#'+this.uuid+' .cube-wrap')).style.display='none';
	}
}
